...
const SkillTree: React.FC = () => {
    ...
    const skillPoints = useAppStore(state => state.settings.skillPoints);
    const unlockedSkills = useAppStore(state => state.settings.unlockedSkills);
    const unlockSkill = useAppStore(state => state.unlockSkill);
    
    const [selectedSkill, setSelectedSkill] = useState<Skill | null>(null);

    const branches = useMemo(() => {
        const tree: Record<string, Skill[]> = { TRADE: [], LOGISTICS: [], INFLUENCE: [] };
        skillsData.forEach(skill => { tree[skill.branch].push(skill); });
        return tree;
    }, []);
        ...
        <div className="h-full flex flex-col space-y-6 animate-fade-in pb-20">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                    <div className="bg-cyber-purple/20 p-2 rounded-lg text-cyber-purple"><Star size={24} /></div>
                    <div>
                        <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Operator Skills</h2>
                        <p className="text-xs text-gray-500">Unlock permanent system upgrades.</p>
                    </div>
                </div>
            ...
            {selectedSkill && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-cyber-panel border border-cyber-purple rounded-2xl w-full max-w-md p-6 shadow-2xl text-center">
                        <div className="w-16 h-16 bg-cyber-purple/20 text-cyber-purple rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-cyber-purple"><Zap size={32} /></div>
                        ...
                        <div className="flex gap-4"><button onClick={() => setSelectedSkill(null)} className="flex-1 py-3 rounded-lg bg-white/5 hover:bg-white/10 text-white font-bold uppercase text-xs">Cancel</button><button onClick={handleUnlock} className="flex-1 py-3 rounded-lg bg-cyber-purple text-white font-bold uppercase text-xs">Unlock</button></div>
                    </div>
                </div>
            )}
        </div>
